...
#[doc = "Field `APBNWS` writer - Отключение вставки тактов ожидания в процессе обмена по APB при чтении данных (обращении к регистру OTPDAT на чтение). Если такты ожидания отключены (APBNWS=1), то требуется производить опрос флага OTPSTA.BSY после записи адреса в OTPA до тех пор, пока EESTA.BSY не станет равным «0»"]
...
#[doc = "Field `MME` writer - Режим ручного управления включён. В этом режиме автоматизированные операции чтения и записи не выполняются. Используется для тестирования"]
pub type MmeW<'a, REG> = crate::BitWriter<'a, REG>;
impl W {
    #[doc = "Bit 0 - Отключение вставки тактов ожидания в процессе обмена по APB при чтении данных (обращении к регистру OTPDAT на чтение). Если такты ожидания отключены (APBNWS=1), то требуется производить опрос флага OTPSTA.BSY после записи адреса в OTPA до тех пор, пока EESTA.BSY не станет равным «0»"]
    #[inline(always)]
    ...
    #[doc = "Bit 3 - Режим ручного управления включён. В этом режиме автоматизированные операции чтения и записи не выполняются. Используется для тестирования"]
    #[inline(always)]
