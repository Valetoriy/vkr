...
#[doc = "Field `SCLL` reader - Длительность удержания SCL в состоянии логического «0» в режиме «ведущий» t_SCLL = (SCLL+1) x t_PRESC Также используется для генерации задержек t_BUF и t_SU:STA. Примечание: Регистр TIMING должен конфигурироваться, пока интерфейс заблокирован (PE=0)"]
pub type ScllR = crate::FieldReader;
#[doc = "Field `SCLL` writer - Длительность удержания SCL в состоянии логического «0» в режиме «ведущий» t_SCLL = (SCLL+1) x t_PRESC Также используется для генерации задержек t_BUF и t_SU:STA. Примечание: Регистр TIMING должен конфигурироваться, пока интерфейс заблокирован (PE=0)"]
...
#[doc = "Field `SCLDEL` reader - Длительность предустановки данных t_SU:DAT. Задержка между изменением SDA и фронтом SCL. t_SCLDEL = (SCLDEL+1) x t_PRESC Используется для генерации задержек t_SU:DAT. Примечание: Регистр TIMING должен конфигурироваться, пока интерфейс заблокирован (PE=0)"]
pub type ScldelR = crate::FieldReader;
#[doc = "Field `SCLDEL` writer - Длительность предустановки данных t_SU:DAT. Задержка между изменением SDA и фронтом SCL. t_SCLDEL = (SCLDEL+1) x t_PRESC Используется для генерации задержек t_SU:DAT. Примечание: Регистр TIMING должен конфигурироваться, пока интерфейс заблокирован (PE=0)"]
pub type ScldelW<'a, REG> = crate::FieldWriter<'a, REG, 4>;
#[doc = "Field `PRESC` reader - Предварительный делитель частоты I2CCLK. Используется для вычисления значения t_PRESC используемого счётчиками предустановки, удержания, низкого и высокого уровней. t_PRESC = (PRESC+1) x t_I2CCLK. Примечание: Регистр TIMING должен конфигурироваться, пока интерфейс заблокирован (PE=0)"]
pub type PrescR = crate::FieldReader;
#[doc = "Field `PRESC` writer - Предварительный делитель частоты I2CCLK. Используется для вычисления значения t_PRESC используемого счётчиками предустановки, удержания, низкого и высокого уровней. t_PRESC = (PRESC+1) x t_I2CCLK. Примечание: Регистр TIMING должен конфигурироваться, пока интерфейс заблокирован (PE=0)"]
pub type PrescW<'a, REG> = crate::FieldWriter<'a, REG, 4>;
impl R {
    #[doc = "Bits 0:7 - Длительность удержания SCL в состоянии логического «0» в режиме «ведущий» t_SCLL = (SCLL+1) x t_PRESC Также используется для генерации задержек t_BUF и t_SU:STA. Примечание: Регистр TIMING должен конфигурироваться, пока интерфейс заблокирован (PE=0)"]
    #[inline(always)]
    ...
    #[doc = "Bits 20:23 - Длительность предустановки данных t_SU:DAT. Задержка между изменением SDA и фронтом SCL. t_SCLDEL = (SCLDEL+1) x t_PRESC Используется для генерации задержек t_SU:DAT. Примечание: Регистр TIMING должен конфигурироваться, пока интерфейс заблокирован (PE=0)"]
    #[inline(always)]
    pub fn scldel(&self) -> ScldelR {
        ScldelR::new(((self.bits >> 20) & 0x0f) as u8)
    }
    #[doc = "Bits 28:31 - Предварительный делитель частоты I2CCLK. Используется для вычисления значения t_PRESC используемого счётчиками предустановки, удержания, низкого и высокого уровней. t_PRESC = (PRESC+1) x t_I2CCLK. Примечание: Регистр TIMING должен конфигурироваться, пока интерфейс заблокирован (PE=0)"]
    #[inline(always)]
