...
#[doc = "Field `ARR` reader - Значение автоматической перезагрузки ARR - значение автозагрузки для TIMER16. Это значение должно быть строго больше, чем значение CMP\\[15:0\\]. Примечание: Регистр ARR может быть изменён только тогда, когда TIMER16 включён (бит ENABLE установлен в «1»)"]
pub type ArrR = crate::FieldReader<u16>;
#[doc = "Field `ARR` writer - Значение автоматической перезагрузки ARR - значение автозагрузки для TIMER16. Это значение должно быть строго больше, чем значение CMP\\[15:0\\]. Примечание: Регистр ARR может быть изменён только тогда, когда TIMER16 включён (бит ENABLE установлен в «1»)"]
pub type ArrW<'a, REG> = crate::FieldWriter<'a, REG, 16, u16>;
impl R {
    #[doc = "Bits 0:15 - Значение автоматической перезагрузки ARR - значение автозагрузки для TIMER16. Это значение должно быть строго больше, чем значение CMP\\[15:0\\]. Примечание: Регистр ARR может быть изменён только тогда, когда TIMER16 включён (бит ENABLE установлен в «1»)"]
    #[inline(always)]
