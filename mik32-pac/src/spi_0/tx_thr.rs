...
#[doc = "Field `Threshold_of_TX_FIFO` reader - Задаёт уровень, при котором TX_FIFO считается не заполненным и формируется прерывание"]
pub type ThresholdOfTxFifoR = crate::FieldReader<u32>;
#[doc = "Field `Threshold_of_TX_FIFO` writer - Задаёт уровень, при котором TX_FIFO считается не заполненным и формируется прерывание"]
pub type ThresholdOfTxFifoW<'a, REG> = crate::FieldWriter<'a, REG, 32, u32>;
impl R {
    #[doc = "Bits 0:31 - Задаёт уровень, при котором TX_FIFO считается не заполненным и формируется прерывание"]
    #[inline(always)]
