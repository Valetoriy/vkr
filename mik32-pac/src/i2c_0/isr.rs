...
#[doc = "Field `RXNE` reader - Флаг «буфер принятых данных заполнен» (режимы приёма). Устанавливается аппаратно, после записи принятых данных в регистр RXDR. Сбрасывается при чтении RXDR или аппаратно при PE=0."]
pub type RxneR = crate::BitReader;
#[doc = "Field `RXNE` writer - Флаг «буфер принятых данных заполнен» (режимы приёма). Устанавливается аппаратно, после записи принятых данных в регистр RXDR. Сбрасывается при чтении RXDR или аппаратно при PE=0."]
...
#[doc = "Field `NACKF` reader - Флаг «не получено подтверждение» (NACK). Устанавливается аппаратно, после передачи байта. Сбрасывается программной установкой бита NACKCF или аппаратно при PE=0."]
pub type NackfR = crate::BitReader;
#[doc = "Field `STOPF` reader - Флаг детектирования STOP на шине. Устанавливается аппаратно, если интерфейс участвует в передаче. Сбрасывается программной установкой бита STOPCF или аппаратно при PE=0."]
pub type StopfR = crate::BitReader;
#[doc = "Field `TC` reader - Флаг окончания передачи (режим «ведущий»). Устанавливается аппаратно при RELOAD=0, AUTOEND=0, после передачи NBYTES байт. Сбрасывается программной установкой бита START или STOP или аппаратно при PE=0"]
pub type TcR = crate::BitReader;
#[doc = "Field `TCR` reader - Флаг окончания передачи (режим «ведущий» или «ведомый» с установленным битом SBC). Устанавливается аппаратно при RELOAD=1, после передачи NBYTES байт. Сбрасывается записью в NBYTES ненулевого значения или аппаратно при PE=0"]
pub type TcrR = crate::BitReader;
#[doc = "Field `BERR` reader - Флаг ошибки шины Устанавливается аппаратно при детектировании неуместного события START или STOP на шине, если интерфейс участвует в передаче. Не устанавливается в фазе адреса в режиме «ведомый». Сбрасывается программной установкой бита BERRCF или аппаратно при PE=0"]
...
#[doc = "Field `BUSY` reader - Флаг индикации занятой шины. Устанавливается после события START на шине и сбрасывается после события STOP на шине"]
pub type BusyR = crate::BitReader;
#[doc = "Направление передачи (режим «ведомый»). Обновляется при совпадении адреса (ADDR=1)\n\nValue on reset: 0"]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum Dir {
    #[doc = "0: Тип передачи «запись», ведомый переходит в режим приёмника"]
    Write = 0,
...
impl DirR {
    ...
    #[doc = "Тип передачи «запись», ведомый переходит в режим приёмника"]
    #[inline(always)]
...
#[doc = "Field `ADDCODE` reader - Код совпавшего адреса. Обновляется в режиме «ведомый» при совпадении адреса (ADDR=1). В режиме 10-битного адреса содержит заголовок (5b11110) и два старших бита адреса"]
...
impl R {
    ...
    #[doc = "Bit 2 - Флаг «буфер принятых данных заполнен» (режимы приёма). Устанавливается аппаратно, после записи принятых данных в регистр RXDR. Сбрасывается при чтении RXDR или аппаратно при PE=0."]
    #[inline(always)]
    ...
    #[doc = "Bit 4 - Флаг «не получено подтверждение» (NACK). Устанавливается аппаратно, после передачи байта. Сбрасывается программной установкой бита NACKCF или аппаратно при PE=0."]
    #[inline(always)]
    pub fn nackf(&self) -> NackfR {
        NackfR::new(((self.bits >> 4) & 1) != 0)
    }
    #[doc = "Bit 5 - Флаг детектирования STOP на шине. Устанавливается аппаратно, если интерфейс участвует в передаче. Сбрасывается программной установкой бита STOPCF или аппаратно при PE=0."]
    #[inline(always)]
    pub fn stopf(&self) -> StopfR {
        StopfR::new(((self.bits >> 5) & 1) != 0)
    }
    #[doc = "Bit 6 - Флаг окончания передачи (режим «ведущий»). Устанавливается аппаратно при RELOAD=0, AUTOEND=0, после передачи NBYTES байт. Сбрасывается программной установкой бита START или STOP или аппаратно при PE=0"]
    #[inline(always)]
    pub fn tc(&self) -> TcR {
        TcR::new(((self.bits >> 6) & 1) != 0)
    }
    #[doc = "Bit 7 - Флаг окончания передачи (режим «ведущий» или «ведомый» с установленным битом SBC). Устанавливается аппаратно при RELOAD=1, после передачи NBYTES байт. Сбрасывается записью в NBYTES ненулевого значения или аппаратно при PE=0"]
    #[inline(always)]
    pub fn tcr(&self) -> TcrR {
        TcrR::new(((self.bits >> 7) & 1) != 0)
    }
    #[doc = "Bit 8 - Флаг ошибки шины Устанавливается аппаратно при детектировании неуместного события START или STOP на шине, если интерфейс участвует в передаче. Не устанавливается в фазе адреса в режиме «ведомый». Сбрасывается программной установкой бита BERRCF или аппаратно при PE=0"]
    #[inline(always)]
    ...
    #[doc = "Bit 15 - Флаг индикации занятой шины. Устанавливается после события START на шине и сбрасывается после события STOP на шине"]
    #[inline(always)]
    ...
    #[doc = "Bits 17:23 - Код совпавшего адреса. Обновляется в режиме «ведомый» при совпадении адреса (ADDR=1). В режиме 10-битного адреса содержит заголовок (5b11110) и два старших бита адреса"]
    #[inline(always)]
